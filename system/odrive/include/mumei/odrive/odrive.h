 ...
 */
class ODrive : public mumei::GenericComponent {
 protected:
 ...
 public:
  class ODriveConfiguration : public mumei::Configuration {
   public:
    /**
    ...
    ODriveConfiguration(ConfigMap config_map,
      ...
      : mumei::Configuration(config_map,
                             [&valid_keys]() {
  ...
  ODrive(std::unique_ptr<ODriveConfiguration> config, uint32_t get_timeout)
      : mumei::GenericComponent(std::move(config)),

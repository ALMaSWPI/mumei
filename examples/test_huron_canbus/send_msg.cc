...
int main(int argc, char* argv[]) { 
  ...
  // Set input mode & control mode
	...
	msg.isExt = false;
	msg.len = 8;
  can_setSignal<float>(msg, CONTROL_MODE_TORQUE_CONTROL, 0, 32, true);
  can_setSignal<float>(msg, INPUT_MODE_PASSTHROUGH, 32, 32, true);
  ...
  std::cout << "Control mode and input mode set.\n";
	...
	msg.isExt = false;
	msg.len = 8;
  can_setSignal<float>(msg, 0.7, 32, 32, true);
  hcb.send_message(msg);
  std::cout << "Sending torque...\n";

  std::this_thread::sleep_for(2s);

  can_setSignal<float>(msg, 0.0, 32, 32, true);
  hcb.send_message(msg);
  std::cout << "Stopping...\n";

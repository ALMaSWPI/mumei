...
void MumeiNode::AddJointStateProvider(std::shared_ptr<JointStateProvider> jsp,
  ...
  if (!is_odom) {
    ...
    odom_sub_ =
      this->create_subscription<nav_msgs::msg::Odometry>(
        topic,
        10,
        std::bind(&MumeiNode::OdomCallback,
                  this,
                  std::placeholders::_1));
  }
  nq_ += nq;
  nv_ += nv;
  jsp->SetNode(std::static_pointer_cast<MumeiNode>(shared_from_this()));
}

void MumeiNode::AddForceTorqueSensor(
  std::shared_ptr<ForceTorqueSensor> ft_sensor,
  const std::string& topic) {
  size_t idx = wrench_stamped_subs_.size();
  ft_sensor->SetNode(std::static_pointer_cast<MumeiNode>(shared_from_this()));
...
void
MumeiNode::AddJointGroupController(std::shared_ptr<JointGroupController> jgc,
                                   const std::string& topic) {
  size_t idx = float64_multi_array_pubs_.size();
  jgc->SetNode(std::static_pointer_cast<MumeiNode>(shared_from_this()));
  jgc->SetPubIdx(idx);
...
void MumeiNode::JointStateCallback(
...
void MumeiNode::WrenchStampedCallback(
  size_t idx,
  std::shared_ptr<const geometry_msgs::msg::WrenchStamped> msg) {
...
void MumeiNode::PublishFloat64MultiArray(size_t idx,
...
void MumeiNode::Finalize() {
  // joint_state_ = Eigen::VectorXd::Zero(nq_ + nv_);
  joint_state_.resize(nq_ + nv_);
  wrenches_ = std::vector<mumei::Vector6d>(wrench_stamped_subs_.size(),
                                           mumei::Vector6d::Zero());
  finalized_ = true;
}
